...
PHONEME_JAW_MAP = {
    # Vowels (mouth more open)
    "AA": 160, "AE": 160, "AH": 140, "AO": 150,
    "EH": 130, "IH": 120, "IY": 110, "OW": 150,
    "UH": 130, "UW": 140, "AY": 145, "EY": 125,
    
    # Consonants (mouth less open)
    "B": 70, "CH": 90, "D": 80, "DH": 85, "F": 85, 
    "G": 90, "HH": 95, "JH": 90, "K": 90, "L": 100, 
    "M": 60, "N": 65, "P": 80, "R": 95, "S": 90, 
    "SH": 90, "T": 80, "TH": 90, "V": 85, "W": 140, 
    "Y": 100, "Z": 90, "KS": 90, "ER": 110,
    
    "default": 90
}

def phoneme_to_angle(p):
    angle = PHONEME_JAW_MAP.get(p.upper(), PHONEME_JAW_MAP["default"])
    print(f"🔠 Phoneme '{p}' → angle {angle}°")
    return angle

def get_phonemes_from_text(text):
    """Word-based phoneme approximation without espeak"""
    ...
    # === COPY FROM jaw_phoneme_sync.py: Character-to-phoneme fallback ===
    char_to_phoneme = {
    ...
    for word in words:
        ...
        if clean_word in word_phonemes:
            phoneme_list.extend(word_phonemes[clean_word])
        else:
            # Fallback to character-based for unknown words
            for char in clean_word:
                if char.lower() in char_to_phoneme:
                    phoneme_list.append(char_to_phoneme[char.lower()])
...
async def websocket_handler(websocket):
    ...
    print(f"✅ ESP32 connected! Total clients: {len(connected_clients)}")

    try:
        await websocket.send(str(servo_angle))
        print(f"📤 Initial angle sent: {servo_angle}°")

        while True:
            try:
                new_angle = await asyncio.wait_for(angle_queue.get(), timeout=0.1)
                print(f"📥 Got angle from queue: {new_angle}")
                await websocket.send(str(new_angle))
                print(f"📤 Sent angle: {new_angle}°")
                angle_queue.task_done()
            except asyncio.TimeoutError:
                await websocket.send(str(servo_angle))
                print(f"💓 Heartbeat: {servo_angle}°")
...
def set_servo_angle(angle):
    """Set servo angle and send to connected ESP32s"""
    global servo_angle
    new_angle = int(max(0, min(180, angle)))

    if new_angle != servo_angle:
        servo_angle = new_angle
        print(f"🎯 Angle changed to: {servo_angle}°")
        if loop is not None:
            try:
                asyncio.run_coroutine_threadsafe(angle_queue.put(new_angle), loop)
            except RuntimeError:
                print("⚠️ Could not queue angle update - event loop issue")
        else:
            print("⚠️ Event loop not initialized yet")
    return servo_angle

async def start_websocket_server():
    """Start the WebSocket server for ESP32 communication"""
    global loop
...
def animate_jaw_from_text(text):
    """Animate jaw movement based on phonemes from text"""
    phonemes = get_phonemes_from_text(text)
    print(f"📝 Text: '{text}' → Phonemes: {phonemes}")
    for p in phonemes:
        angle = phoneme_to_angle(p)
        set_servo_angle(angle)
        time.sleep(0.12)
    set_servo_angle(90)  # Rest position
...
def text_to_speech(text):
    """Convert text to speech using ElevenLabs SDK with jaw animation"""
    ...
    try:
        ...
        threading.Thread(target=animate_jaw_from_text, args=(text,), daemon=True).start()
...
def main():
    print("🎙️ Initializing KAAZI Voice Assistant with Memory, TTS & Jaw Control...")
    print("📦 Features: Speech Recognition, LLM, TTS, Jaw Animation, WebSocket Server")
    print(f"💾 Memory system: {MEMORY_DIR}")
    print("🦾 Jaw control: WebSocket server for ESP32 servo control")
    ...
    # Start WebSocket server in background thread for ESP32 jaw control
    print("🚀 Starting WebSocket server for jaw control...")
    ...
    # === COPY FROM main.py: All event handler registrations ===
    ...
    try:
        ...
        print("🦾 Jaw animation enabled - Connect ESP32 to ws://your-ip:8765")
